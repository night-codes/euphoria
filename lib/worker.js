		...
		async.series([
			...
			function(callback){
				fs.exists(fln, function (e) {
					exists = e;
					if (!e) logger.dev("Мain db file not found");
			...
			function(callback){
				fs.exists(tmp, function (e) {
					exists_tmp = e;
					if (e) logger.dev("Temporary db file found!");
				...
				if(!exists && !exists_tmp) {
					...
					fs.readFile(tmp, {encoding: 'utf8'}, function(err, d) {
						try {
							...
							logger.dev("Error from loading temporary file (" + e + ")");
							if (exists){
								logger.dev("Try load main file...");
